...
# ADABoost
    ...
    def BM(self):
        ...
        Tdatos = pd.DataFrame(datos, index=["AlgKnn", "AlgDT", "AlgRF", "AlgXGBoost", "AlgADABoost"], columns=["PG","EG","PP","PN"], dtype=float)
...
class Regresion(analisisEDA):
    ...
    def errores(self, y_true, y_predicted):
        ...
        R2 = 1 - (SS_res / SS_tot)
        error = pd.DataFrame()
        nombres = ['RMSE', 'MAE', 'ER', 'R2']
        errores = [RMSE, MAE, ER, R2]
        error['Tipo'] = nombres
        error['Error'] = errores
        return error
    ...
    def dataCleaning(self):
        self.__df = self.__df.dropna().copy()
    ...
    def dataCleaning2(self):
        self.__df = self.__df.dropna().copy()
        self.__df['floors'] = self.__df['floors'].astype('category')
        self.__df['waterfront'] = self.__df['waterfront'].astype('category')
        self.__df['view'] = self.__df['view'].astype('category')
        self.__df['condition'] = self.__df['condition'].astype('category')
        self.__df['grade'] = self.__df['grade'].astype('category')
        self.__df['zipcode'] = self.__df['zipcode'].astype('category')
        self.__df['floors'] = self.__df['floors'].cat.codes
        self.__df['waterfront'] = self.__df['waterfront'].cat.codes
        self.__df['view'] = self.__df['view'].cat.codes
        self.__df['condition'] = self.__df['condition'].cat.codes
        self.__df['grade'] = self.__df['grade'].cat.codes
        self.__df['zipcode'] = self.__df['zipcode'].cat.codes
        return self.__df
    ...
    def RegLinSimp(self, X_train, X_test, y_train, y_test):
        ...
        err_RegSimp = self.errores(y_test,predicciones)
        return err_RegSimp
    ...
    def RegLinMult(self, X_train, X_test, y_train, y_test):
        ...
        print(predicciones[0:3,])
        err_RegMul = self.errores(y_test,predicciones)
        return err_RegMul

    def RegLasso(self, X_train, X_test, y_train, y_test, scale=True):
        if scale:
        ...
        err_lasso = self.errores(y_test, predicciones)
        df_coeficientes = pd.DataFrame({'predictor': X_train.columns if not isinstance(X_train, np.ndarray) else [f'x{i}' for i in range(X_train.shape[1])],'coef': modelo_lasso.coef_.flatten()})
        print(df_coeficientes)
        return err_lasso
    
    def RegLassoCV(self, X_train, X_test, y_train, y_test, scale=True):
        if scale:
            scaler = StandardScaler()
            X_train = scaler.fit_transform(X_train)
            X_test = scaler.transform(X_test)
        ...
        err_lasso2 = self.errores(y_test, predicciones)
        df_coeficientes = pd.DataFrame({'predictor': X_train.columns if not isinstance(X_train, np.ndarray) else [f'x{i}' for i in range(X_train.shape[1])],'coef': modelo_lasso2.coef_.flatten()})
        print(df_coeficientes)
        return err_lasso2


    def ComparacionLasso(self, X_train, X_test, y_train, y_test, scale=True):
        err_lasso = self.RegLasso(X_train, X_test, y_train, y_test, scale=scale)
        err_lasso2 = self.RegLassoCV(X_train, X_test, y_train, y_test, scale=scale)
        comparacion = pd.DataFrame([err_lasso.Error, err_lasso2.Error])
        comparacion.columns = ['RMSE','MAE','ER','R2']
        comparacion.index = ['Lasso', 'Lasso CV']
        return comparacion

    
    def RegRidge(self, X_train, X_test, y_train, y_test, scale=True):
        if scale:
            scaler = StandardScaler()
            X_train = scaler.fit_transform(X_train)
            X_test = scaler.transform(X_test)
        ...
        err_Ridge = self.errores(y_test, predicciones)
        df_coeficientes = pd.DataFrame({'predictor': X_train.columns if not isinstance(X_train, np.ndarray) else [f'x{i}' for i in range(X_train.shape[1])],'coef': modelo_Ridge.coef_.flatten()})
        print(df_coeficientes)
        return err_Ridge
    
    def RegRidgeCV(self, X_train, X_test, y_train, y_test, scale=True):
        if scale:
            scaler = StandardScaler()
            X_train = scaler.fit_transform(X_train)
            X_test = scaler.transform(X_test)
        modelo = RidgeCV(alphas=np.logspace(-10, 2, 200), fit_intercept=True)
        modelo.fit(X=X_train, y=y_train)
        modelo_Ridge2 = Ridge(alpha=modelo.alpha_)
        modelo_Ridge2.fit(X=X_train, y=y_train)
        predicciones = modelo_Ridge2.predict(X_test)
        err_Ridge2 = self.errores(y_test, predicciones)
        df_coeficientes = pd.DataFrame({'predictor': X_train.columns if not isinstance(X_train, np.ndarray) else [f'x{i}' for i in range(X_train.shape[1])],'coef': modelo_Ridge2.coef_.flatten()})
        print(df_coeficientes)
        return err_Ridge2

    
    def ComparacionRidge(self, X_train, X_test, y_train, y_test, scale=True):
        err_Ridge = self.RegRidge(X_train, X_test, y_train, y_test, scale=scale)
        err_Ridge2 = self.RegRidgeCV(X_train, X_test, y_train, y_test, scale=scale)
        comparacion = pd.DataFrame([err_Ridge.Error, err_Ridge2.Error])
        comparacion.columns = ['RMSE','MAE','ER','R2']
        comparacion.index = ['Ridge', 'Ridge CV']
        return comparacion
    ...
    def SVM(self, X_train, X_test, y_train, y_test):
        ...
        err1 = self.errores(y_test,predicciones1)
        err2 = self.errores(y_test,predicciones2)
        err3 = self.errores(y_test,predicciones3)
    ...
    def DecisionTreeReg(self, X_train, X_test, y_train, y_test):
        ...
        err_arbol = self.errores(y_test,predicciones)
        return err_arbol
    ...
    def RandomForestReg(self, X_train, X_test, y_train, y_test):
        ...
        err_Bosque = self.errores(y_test,predicciones)
        return err_Bosque
    
    def XGBoostingReg(self, X_train, X_test, y_train, y_test):
        modelo_Potenciacion = GradientBoostingRegressor(n_estimators = 500, max_depth = 4, min_samples_split = 5)
        modelo_Potenciacion.fit(X=X_train, y=y_train)
        predicciones = modelo_Potenciacion.predict(X=X_test)
        err_Potenciacion = self.errores(y_test,predicciones)
        return err_Potenciacion
    
    def ComparacionFinal(self, X_train, X_test, y_train, y_test):
